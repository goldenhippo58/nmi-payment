...
// Microservice mode: Start the HTTP server
func startMicroservice() {
    ...
    securityMiddleware := middleware.NewSecurityMiddleware(100) // 100 requests per minute
    ...
    go func() {
        metrics.LogInfo("Starting server on port 8080")
        if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
            metrics.LogError(fmt.Errorf("server error: %v", err))
        }
    }()
